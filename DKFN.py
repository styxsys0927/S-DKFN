...
class DKFN(nn.Module):
    ...
    def __init__(self, K, A, feature_size, pred_size=1, Clamp_A=True):
        # GC-LSTM
        '''
        Args:
            K: K-hop graph
        ...
        self.fc_mo = nn.Linear(1, self.pred_size)
    ...
    def forward(self, input, Hidden_State, Cell_State, rHidden_State, rCell_State):
        ...
        # Kalman Filtering
        var1, var2 = torch.var(input), torch.var(gc) # var of original input and input after GCN
    ...
    def loop(self, inputs):
        ...
        # print('before loop', Hidden_State.size(), Cell_State.size(), rHidden_State.size(), rCell_State.size())
        ...
        pred = self.fc_mo(pred.unsqueeze(1).transpose(1, 2)).transpose(1, 2)
