...
def TrainDKFN(train_dataloader, valid_dataloader, A, K=3, num_epochs=1):
    ...
    learning_rate = 1e-4
    dkfn = DKFN(K, torch.Tensor(A), A.shape[0], pred_size)
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_dataloader:
            ...
            labels = labels ### to be work on for multi-step prediction
            pred = dkfn.loop(inputs)

            # print(inputs.size(), labels.size(), pred.size())
            # print('----------------------- model')
            #
            # for name, param in dkfn.named_parameters():
            #     p = param.detach().cpu().data.numpy()
            #     print(name, p)

            loss_train = loss_MSE(pred, labels)

            # print('------------------', trained_number)
            # print(pred.cpu().data.numpy(), labels.cpu().data.numpy())
            # print('train: ', loss_train.cpu().data.numpy())
            ...
            # print(inputs_val.size(), labels_val.size(), pred.size())
            loss_valid = loss_MSE(pred, labels_val)
            # record here
            losses_valid.append(loss_valid.data)

            # print('valid: ', loss_valid.cpu().data.numpy())
        ...
        if best_loss >= loss_epoch.mean():
            ...
            torch.save(dkfn.state_dict(), './models/dkfn_pems3000_'+str(epoch))
